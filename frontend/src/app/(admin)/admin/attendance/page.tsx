...
const getErrorMessage = (error: any, context: string) => {
  ...
  const statusMappings: Record<number, { title: string; message: string }> = {
    ...
    403: { title: '❌ Akses Ditolak', message: 'Anda tidak memiliki akses untuk melakukan operasi ini. Pastikan Anda adalah admin.' },
...
export default function AdminAttendancePage() {
  ...
  // Redirect jika bukan admin atau super admin
  useEffect(() => {
  ...
  const loadEvents = async () => {
    try {
      setLoading(true);
      const response = await ApiService.getAdminEvents({ page: 1, limit: 100 });
      if (response.success) {
  ...
  const loadAttendanceData = async () => {
    try {
      setLoading(true);
      const response = await ApiService.getEventAttendance(selectedEventId);
      if (response.success) {
  ...
  const detectEventFromToken = async (token: string) => {
    try {
      ...
      const response = await ApiService.detectEventFromToken(token);
  ...
  const handleQRScan = async () => {
    ...
    try {
      ...
      const response = await ApiService.adminCheckInParticipant(selectedEventId, qrDataToSend);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100">
      <div className="max-w-7xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-light text-blue-600 mb-3">
            Attendance Management
          </h1>
          <p className="text-gray-600 text-lg font-light">Kelola kehadiran peserta event dengan QR code scanner</p>
        </div>
          ...
          <div className="lg:col-span-1">
            <Card className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
              ...
              <CardContent className="space-y-4">
                {/* Event Selection */}
                <div>
                  <label className="block text-sm font-light mb-2">Pilih Event</label>
